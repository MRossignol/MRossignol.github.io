...
tarball.TarWriter = class {
  ...
  addFileBlob(name, blob, opts) {
    blob.arrayBuffer().then(arr => this.addFileArrayBuffer(name, arr, opts));
  }
  ...
  _createBuffer(from, nb) {
    let tarDataSize = 0;
    for(let i = from; i < Math.min(from+nb, this.fileData.length); i++) {                        
  ...
  download(filename_base) {
    this.doDownload(filename_base, 0);
  }

  doDownload(filename_base, from) {
    if (from >= this.fileData.length) return;
    if (this.urlToClear) {
      URL.revokeObjectURL(this.urlToClear);
      this.urlToClear = null;
    }
    let blob = this.writeBlob(from, 1000);
    let $downloadElem = document.createElement('a');
    let url = URL.createObjectURL(blob);
    $downloadElem.href = url;
    this.urlToClear = url;
    $downloadElem.download = `${filename_base}_${from/1000}.tar`;
    $downloadElem.style.display = "none";
    document.body.appendChild($downloadElem);
    $downloadElem.click();
    document.body.removeChild($downloadElem);
    setTimeout(() => this.doDownload(filename_base, from+1000), 10000);
  }

  writeBlob(from, nb) {
    return new Blob([this.write(from, nb)], {"type":"application/x-tar"});
  }

  write (from, nb) {
    this._createBuffer(from, nb);
    let offset = 0;
    let filesAdded = 0;
    for(let fileIdx = from; fileIdx < Math.min(from+nb, this.fileData.length); fileIdx++) {
      let fdata = this.fileData[fileIdx];
      this.fileData[fileIdx] = null;
      // write header
